...
pick_port() {
  if [[ -n "${TASK_7_R5_PORT:-}" ]]; then
    port_is_free "${TASK_7_R5_PORT}" || fail "requested TASK_7_R5_PORT is already in use: ${TASK_7_R5_PORT}"
    printf '%s\n' "${TASK_7_R5_PORT}"
    return 0
  fi
  ...
  fail "could not find a free port for task-7-r5 backend"
}
...
DB_PATH="$(mktemp -t task-7-r5-live-db.XXXXXX)"
SERVER_LOG="$(mktemp -t task-7-r5-server-log.XXXXXX)"
SENTINEL_OUT="$(mktemp -t task-7-r5-sentinel.XXXXXX)"
ORIGINAL_FILE_OUT="$(mktemp -t task-7-r5-original-path.XXXXXX)"
DM_ATTACHMENT_ID_OUT="$(mktemp -t task-7-r5-dm-attachment-id.XXXXXX)"
GROUP_ATTACHMENT_ID_OUT="$(mktemp -t task-7-r5-group-attachment-id.XXXXXX)"
BOB_TOKEN_OUT="$(mktemp -t task-7-r5-bob-token.XXXXXX)"
UPLOAD_WIRE_FILE_OUT="$(mktemp -t task-7-r5-upload-wire-path.XXXXXX)"
LIVE_ORIGINAL_FILE_OUT="$(mktemp -t task-7-r5-live-original-path.XXXXXX)"
LIVE_DM_DOWNLOAD_OUT="$(mktemp -t task-7-r5-live-dm-download-path.XXXXXX)"
LIVE_GROUP_DOWNLOAD_OUT="$(mktemp -t task-7-r5-live-group-download-path.XXXXXX)"
STORED_BLOB="$(mktemp -t task-7-r5-stored-blob.XXXXXX)"
DOWNLOADED_BLOB="$(mktemp -t task-7-r5-downloaded-blob.XXXXXX)"
PORT="$(pick_port)"
...
if [[ "${swift_status}" -ne 0 ]]; then
  fail "swift build or attachment-scoped test run failed"
fi
...
require_swift_class_count "${swift_output}" "FileCryptoTests" "6"
require_swift_class_count "${swift_output}" "AttachmentDescriptorTests" "4"
require_swift_class_count "${swift_output}" "HTTPAttachmentServiceTests" "4"
require_swift_class_count "${swift_output}" "DMCoordinatorTests" "10"
require_swift_class_count "${swift_output}" "LiveAttachmentE2ETests" "2"

require_swift_test_ok "${swift_output}" "testLiveAttachmentDMAndGroupRoundTripStoresOnlyCiphertext"
require_swift_test_ok "${swift_output}" "testAlreadySubscribedRecipientReceivesLiveAttachmentsAndWritesByteIdenticalDownloads"

excluded_swift_suites=("LiveGroupE2ETests" "LiveOfflineDeliveryE2ETests")
for excluded_suite in "${excluded_swift_suites[@]}"; do
  if grep -q "${excluded_suite}" <<<"${swift_output}"; then
    fail "excluded Swift suite appeared in scoped output: ${excluded_suite}"
  fi
done
